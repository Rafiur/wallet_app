...
func (l *apiLogger) getLoggerLevel() zapcore.Level {
	levelStr := l.cfg.LogLevel
	level, exist := loggerLevelMap[levelStr]
	if !exist {
		return zapcore.DebugLevel
	}
	return level
...
func (l *apiLogger) InitLogger() {
	...
	// Respect disable flags (though Zap doesn't disable caller/stacktrace directly, we can omit AddCaller)
	logger := zap.New(core, zap.AddCaller(), zap.AddCallerSkip(1))

	// You can add more options here if needed, e.g., zap.WithCaller(!l.cfg.LogDisableCaller)

	l.logger = logger
}

// All the method implementations stay exactly the same as before
...
func (l *apiLogger) Errorf(template string, args ...interface{}) {
	l.logger.Sugar().Errorf(template, args...) // ← Fixed the previous fmt.Println hack
}
